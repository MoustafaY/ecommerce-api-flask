...
## Shopping cart does not clear after order, track sum and shopping cart after order
    ...
    try:
        ...
        for product in products:
            if 'name' not in product or 'quantity' not in product or 'price' not in product:
                raise BadRequest
            newProduct = productService.create(name=product.get("name"), quantity=product.get("quantity"), price=product.get("price"), supplierId=uuid.UUID(id))
            productsCreated.append(newProduct)
        return jsonify(productsCreated), 200
    except UnmappedInstanceError:
        return jsonify({"message": "Supplier not found"}), 404
    except BadRequest:
        return jsonify({"message": "Invalid input"}), 400
...
def login():
    try:
        ...
        if 'user' not in data or 'email' not in data or 'password' not in data:
            raise BadRequest
        if user == 'customer':
            currentUser = customerService.get_by_email(email, password)
        else:
            currentUser = supplierService.get_by_email(email, password)
        access_token = create_access_token(identity=currentUser.id, expires_delta=timedelta(minutes=10))
        return jsonify({'message': f"Hello {currentUser.name}, you are logged in!", 'token': access_token}), 200
    except BadRequest:
        return jsonify({"message": "invalid input"}), 400
    except NoResultFound:
